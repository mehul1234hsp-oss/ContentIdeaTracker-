// frontend/src/components/CreateIdeaPage.js
...
export default function CreateIdeaPage({ onSave, onCancel }) {
    const [editName, setEditName] = useState("");
    const [editText, setEditText] = useState("");
    const [editChannel, setEditChannel] = useState("Instagram");
    ...
    return (
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: C.bg, color: C.text, fontFamily: "Inter,sans-serif" }}>
            ...
            <nav style={{
                ...
                {/* Cancel — ghost pill, bright readable text */}
                <button
                    onClick={onCancel}
                    style={btn({
                ...
                {/* Save Idea — crimson gradient capsule with glow */}
                <button
                    ...
                    style={btn({
                        ...
                        background: saving
                            ? "#c0c0c0"
                            : "linear-gradient(135deg, #E8334A, #ff6347)",
            ...
            <div style={{ flex: 1, display: "flex", justifyContent: "center", padding: "30px 20px", overflowY: "auto" }}>
                <div style={{ width: "100%", maxWidth: "700px", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: "30px", boxShadow: "0 10px 40px rgba(0,0,0,0.03)" }}>
                    ...
                    <div style={{ display: "flex", gap: "20px", marginBottom: "20px", flexWrap: "wrap" }}>
                        ...
                        <div style={{ flex: 1, minWidth: "200px" }}>
                            <div style={{ fontSize: ".75rem", fontWeight: 700, color: C.muted2, textTransform: "uppercase", marginBottom: 8, letterSpacing: 1 }}>Status</div>
                            <div style={{ display: "flex", gap: "8px" }}>
                                {[
                                    { value: "idea", label: "Idea", color: "#3b82f6", bg: "rgba(59,130,246,0.1)" },
                                    { value: "draft", label: "Draft", color: "#f59e0b", bg: "rgba(245,158,11,0.1)" },
                                    { value: "published", label: "Published", color: "#10b981", bg: "rgba(16,185,129,0.1)" },
                                ].map(opt => (
                                    <button
                                        key={opt.value}
                                        type="button"
                                        ...
                                        style={{
                                            ...
                                            padding: "12px 8px",
                                            borderRadius: 10,
                                            border: `2px solid ${editStatus === opt.value ? opt.color : C.border}`,
                                            background: editStatus === opt.value ? opt.bg : C.surface,
                                            color: editStatus === opt.value ? opt.color : C.muted2,
                                            ...
                                            letterSpacing: "0.3px",
                                            boxShadow: editStatus === opt.value ? `0 0 0 4px ${opt.color}18` : "none",
                                        }}
                                    >
                                        {opt.label}
                                    </button>
